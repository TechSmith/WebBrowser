...
public:
   ...
   //return S_FALSE so the default is called
   ...
   HRESULT STDMETHODCALLTYPE TranslateAccelerator(LPMSG pMsg, const GUID *, DWORD);
   ...
   HRESULT STDMETHODCALLTYPE TranslateUrl(DWORD, OLECHAR *, OLECHAR **) { return E_NOTIMPL; }
...
protected:
   ...
   CUIHandler m_UIHandler; //intercept the events for the web page
public:
   ...
   // *** CAUTION *** 
      // Must call hookuihandler and then unhookuihandler later. If you do not call unhook,
      // you will not notice a side effect but sooner or later will realize that your app randomly crashes
      // this behavior could be seen sometimes when locking the computer and then unlocking it
   void HookUIHandler() { SetUIHandler(&m_UIHandler); }
   void UnHookUIHandler() { SetUIHandler(NULL); }

   void SetUIHandler(IDocHostUIHandler * pHandler)
   {
      CComPtr<IDispatch> pDisp = get_Document();

      if (pDisp)
      {
         CComQIPtr<ICustomDoc> pCustomDoc(pDisp);

         if (pCustomDoc)
            pCustomDoc->SetUIHandler(pHandler);
      }
   }
